...
module.exports = function (RED) {
  ...
  function HaiwellReadBit (config) {
    ...
    this.haiwellComponent = config.haiwellComponent || 'X'

    let offset = 0
    let isReadOnly = false
    switch (this.haiwellComponent) {
      case 'X': offset = 0; isReadOnly = true; break
    ...
    this.dataType = isReadOnly ? 'Input' : 'Coil'
    ...
    node.onModbusReadDone = function (resp, msg) {
      if (node.showStatusActivities) {
        setNodeStatusWithTimeTo('read done adr:' + node.adr)
      }
    ...
    node.modbusPollingRead = function () {
      ...
      if (node.showStatusActivities) {
        setNodeStatusWithTimeTo('polling adr:' + node.adr)
      }
    ...
    function sendMessage (values, response, msg) {
      ...
      let convertedValue = values // bit data is already an array of booleans
      if (Array.isArray(values) && values.length > node.quantity) {
        convertedValue = values.slice(0, node.quantity)
      }
  ...
  RED.nodes.registerType('haiwell-read-bit', HaiwellReadBit)
