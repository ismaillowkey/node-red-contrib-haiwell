...
module.exports = function (RED) {
  ...
  const internalDebugLog = require('debug')('contribHaiwell:write-multi-word')

  function HaiwellWriteMultiWord (config) {
    ...
    node.on('input', function (msg) {
      ...
      try {
        ...
        // Determine how many registers we need PER ITEM
        let requiredRegistersPerItem = 1
        switch (node.targetType) {
          case 'int16':
          case 'uint16':
            requiredRegistersPerItem = 1
            break
          case 'int32':
          case 'uint32':
          case 'float32':
            requiredRegistersPerItem = 2
            break
          case 'int64':
          case 'uint64':
          case 'double64':
            requiredRegistersPerItem = 4
            break
        }

        const totalRegisters = node.quantity * requiredRegistersPerItem
        const outArray = []
        ...
        for (let i = 0; i < node.quantity; i++) {
          ...
          if (node.targetType === 'int64' || node.targetType === 'uint64') {
            ...
            value = Number(value)
            if (isNaN(value)) throw new Error(`Invalid number input at index ${i}`)
          }

          // Allocate a buffer and write the numeric value into it
          const buf = Buffer.alloc(requiredRegistersPerItem * 2)
          ...
          if (requiredRegistersPerItem === 1) {
            outArray.push(buf.readUInt16BE(0))
          } else if (requiredRegistersPerItem === 2) {
          ...
          } else if (requiredRegistersPerItem === 4) {
            if (node.endianness === 'ABCD') {
              outArray.push(buf.readUInt16BE(0))
              outArray.push(buf.readUInt16BE(2))
        ...
        const newMsg = node.buildNewMessageObject(node, origMsgInput, outArray, totalRegisters)
  ...
  RED.nodes.registerType('haiwell-write-multi-word', HaiwellWriteMultiWord)
}
